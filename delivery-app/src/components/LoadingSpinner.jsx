...
export default function LoadingSpinner({ message = 'Loading ANANTA TRADERS...', skeleton = false }) {
  if (skeleton) {
    return (
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[280px] p-8 text-center">
      <div className="relative mb-4">
        {/* Ambient pulse ring */}
        <div className="w-16 h-16 rounded-2xl bg-brand-500/15 border border-brand-500/30 flex items-center justify-center shadow-lg shadow-brand-500/10 animate-pulse">
          <Truck className="w-7 h-7 text-brand-400 animate-bounce" />
        </div>
        <Loader2 className="w-20 h-20 text-brand-500/30 animate-spin absolute -top-2 -left-2" />
      </div>
      <span className="text-sm font-bold text-slate-200 tracking-wide font-display">{message}</span>
      <span className="text-xs text-slate-500 mt-1 font-medium">Connecting to live dispatch network</span>
    </div>
  );
}
