...
export default function Login() {
  ...
  // Role: null | 'ADMIN' | 'DEALER' | 'USER' | 'DRIVER'
  const [selectedRole, setSelectedRole] = useState(initialRole || null);

  // Sync state if URL searchParams change
  useEffect(() => {
    const roleInQuery = searchParams.get('role');
    if (roleInQuery && ['ADMIN', 'DEALER', 'USER', 'DRIVER'].includes(roleInQuery.toUpperCase())) {
  ...
  const getRoleMetadata = () => {
    switch (selectedRole) {
      case 'DRIVER':
        return {
          title: 'Fleet Driver Login',
          subtitle: 'Live Google Maps Navigation & Delivery Tasks',
          icon: Navigation,
          badge: 'Driver Portal',
          badgeStyle: 'bg-sky-500/10 text-sky-400 border-sky-500/20',
          borderColor: 'focus:border-sky-500',
          btnBg: 'bg-sky-500 hover:bg-sky-400 text-slate-950 shadow-sky-500/20'
        };
      case 'ADMIN':
        return {
      ...
      default:
        return {
          ...
          btnBg: 'bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-emerald-500/20'
        };
    }
  };
