...
export default function UserLayout() {
  ...
  const navItems = [
    ...
    { name: 'Create Order', path: '/user/create-order', icon: PlusCircle, highlight: true },
    { name: 'My Orders', path: '/user/orders', icon: Package },
    { name: 'Notifications', path: '/user/notifications', icon: Bell, badge: unreadCount },
    { name: 'Profile & GST', path: '/user/profile', icon: User }
  ];
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row text-slate-100">
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-bold text-base font-display text-white">ANANTA <span className="text-brand-400">TRADERS</span></span>
        </Link>

        <div className="flex items-center gap-2">
          <Link to="/user/notifications" className="relative p-2 text-slate-400 hover:text-white">
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-slate-900/95 md:bg-slate-900 border-r border-slate-800/80 flex flex-col justify-between transition-transform duration-300 ${
      ...
      >
        <div>
          ...
          <div className="p-4 mx-3 my-4 bg-slate-950/60 rounded-xl border border-slate-800">
            ...
            <div className="text-[11px] text-brand-400 font-medium capitalize mt-0.5">
              {user?.userType || 'Customer'} • {user?.mobile}
            </div>
            {user?.gstNumber && (
              <div className="text-[10px] text-slate-500 mt-1 font-mono">GST: {user.gstNumber}</div>
            )}
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge > 0 && (
                    <span className="px-2 py-0.5 text-xs rounded-full bg-brand-500 text-slate-950 font-bold">
                      {item.badge}
                    </span>
                  )}
                </Link>
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          <Link
            to="/"
            className="flex items-center justify-between text-xs text-slate-400 hover:text-slate-200 px-3 py-2 rounded-lg hover:bg-slate-800/60"
          >
          ...
          <button
            onClick={logout}
            className="w-full flex items-center gap-2.5 px-3.5 py-2 rounded-xl text-sm font-semibold text-rose-400 hover:bg-rose-950/40 border border-rose-900/30 transition-colors"
          >
